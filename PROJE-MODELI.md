...
| 13 | Final testi geçme ve tekrar mantığı | **Testi ilk kez tamamlamak** madalyayı kazandırır; doğru sayısı şartı YOK. Test bir engel değil, hatırlama+kutlama anıdır. Çocuk kaç doğru yaparsa yapsın testi bitirince madalyayı alır ve sonraki kitap açılır. Doğru sayısı yalnızca veli panelinde sayısal görünür; çocuk ekranında yanlışlar utandırıcı vurgulanmaz. Tamamlanan final yeniden açılabilir; bu giriş açıkça **Alıştırma** olarak gösterilir ve `user_progress`, kayıtlı final sonucu, madalya veya `updated_at` alanlarını değiştirmez. ("Derecelendirmeme" felsefesinin gereği.) | Yok |
